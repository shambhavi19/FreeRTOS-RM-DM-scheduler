...
static void prvPeriodicTaskCode( void *pvParameters )
{
    ...
    */
	...
	{	
		...
		Serial.print((pxThisTask->xLastWakeTime)*portTICK_PERIOD_MS);
		Serial.flush();
		...
		TaskHandle_t *pxCreatedTask, TickType_t xPhaseTick, TickType_t xPeriodTick, TickType_t xMaxExecTimeTick, TickType_t xDeadlineTick )
{
   ...
   pxNewTCB->xPriorityIsSet = pdTRUE;
	...
	// Serial.println("");
	Serial.print("New task created: ");
	Serial.print(pxNewTCB->pcName);
	Serial.print("  with WCET: ");
  	Serial.print((pxNewTCB->xMaxExecTime)*portTICK_PERIOD_MS);
	Serial.print("  with Period: ");
  	Serial.print((pxNewTCB->xPeriod)*portTICK_PERIOD_MS);
	Serial.print("  with Deadline: ");
  	Serial.println((pxNewTCB->xRelativeDeadline)*portTICK_PERIOD_MS);
	Serial.flush();
}
...
static void prvSetFixedPriorities( void )
{
	...
	{
		Serial.println("  ");
		Serial.print("Deadline missed by: ");
		...
		// Serial.println("  ");
		Serial.print("Task recreated: ");
		Serial.println(pxTCB->pcName);
		/* Need to reset next WakeTime for correct release. */
		/* your implementation goes here */
		pxTCB->xReleaseTime = pxTCB->xPeriod + pxTCB->xLastWakeTime;
	...
	{ 
		...
		{
			pxTCB->xAbsoluteDeadline = pxTCB->xRelativeDeadline + pxTCB->xLastWakeTime;
	...
	{
		Serial.println("");
		Serial.print("Worst case execution time exceeded by: ");
		Serial.print(pxCurrentTask->pcName);
		// Serial.print(" ");
		// Serial.print(pxCurrentTask->xExecTime);
		// Serial.print(" ");
		// Serial.println(xTickCount);
	...
	static void prvSchedulerFunction( void *pvParameters )
	{
		...
		{ 
			...
			#if( schedOVERHEAD_ENABLED == 1 )
				//Scheduler overhead using _delay_ms()
				_delay_ms(schedOVERHEAD_IN_MS);
				// Serial.println("");
				// Serial.println("Scheduler delay of 100ms  ");
			#endif
	...
	static void prvCreateSchedulerTask( void )
	{
		Serial.println("Scheduler task created");
...
{
	#if( schedSCHEDULING_POLICY == schedSCHEDULING_POLICY_RMS )
		Serial.println("................................RMS policy..............................");
		prvSetFixedPriorities();	
	#endif /* schedSCHEDULING_POLICY */

	#if( schedSCHEDULING_POLICY == schedSCHEDULING_POLICY_DMS )
		Serial.println("................................DMS policy..............................");
		prvSetFixedPriorities();	
	#endif /* schedSCHEDULING_POLICY */
